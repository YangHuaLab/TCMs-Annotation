...
def resource_path(relative_path):
    """根据程序是否打包，动态获取资源路径"""
    try:
        # PyInstaller 打包后会将资源文件放在临时目录 _MEIPASS 下
...
def unite_mzrt1(file_path1):
    try:
        ...
        print(f"Error in unite_mzrt1: {e}")  # 打印异常信息
        return pd.DataFrame(columns=['RT,m/z', 'fraction'])
...
def connect_to_gephi(file_path1, file_path2):
    try:
        ...
        print(f"Error in connect_to_gephi: {e}")  # 打印异常信息
...
class Window4(QWidget, Ui_Form4):
    ...
    def import_file1(self):
        ...
        if self.file_path1 == '':
            ...
            print(f"File1 path: {self.file_path1}")  # 打印文件路径
            self.importbrowser1.setText(f"{self.file_path1.split('/')[-1]} imported.")
            self.importbrowser1.repaint()
    ...
    def import_file2(self):
        ...
        if self.file_path2 == '':
            ...
            print(f"File2 path: {self.file_path2}")  # 打印文件路径
    ...
    def start_process1(self):
        try:
            ...
            print(f"Error1 in start_process1: {e2}")  # 打印异常信息
    #
        try:
            ...
            print(f"Error2 in start_process1: {e2}")  # 打印异常信息

    def start_process2(self):
        try:
            connect_to_gephi(self.file_path1, self.file_path2)
        except Exception as e:
            print(f"Error in start_process2: {e}")  # 打印异常信息
...
class HelloWindow(QWidget, helloui):
    ...
    def init_ui(self):
        ...
        image_path = resource_path(r"icon\软件封面.jpg")
